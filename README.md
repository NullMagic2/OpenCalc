...
The Linux History boundary no longer exposes wxGTK's desktop-themed sash as a warm grey strip. OpenCalc places its own neutral, etched separator over the native sash so the warm desktop-theme strip is not visible. Buildfix86 supersedes the temporary custom drag handler by making this decoration pointer-transparent and restoring native sash resizing. This is confined to the Linux frontend; Windows keeps its existing native splitter and DPI-aware decoration path.
...
## Native History drag, Statistics toggle, and Graph editor focus — buildfix86

The Linux History boundary is again driven by the real `wxSplitterWindow` sash. The visible etched overlay remains the exact neutral `RGB(240,240,240)` surface, but it is now pointer-transparent and centered over a normal-width native sash. GTK/wxWidgets therefore owns the drag cursor and sash movement, while OpenCalc continues to normalize the Calculator pane and persist the resulting History width.

The modeless Statistics Box now installs the same Calculator character handler on its top-level frame, panel, and focusable data list. Digits, operators, memory shortcuts, and Scientific calculator keys therefore continue to operate the main calculator while Statistics is active on both Windows and Linux. Pressing **Sta** now closes and destroys an existing Statistics Box; pressing it again creates, centers, and shows a fresh box while preserving the statistical dataset stored in the Calculator model.

The Graph Function editor uses square classic entry chrome on Linux and no longer receives programmatic focus when the Graph pane is shown. It remains a normal editable field and receives focus through ordinary direct user interaction.
## Native Linux History sash and keypad gutter — buildfix87

- Linux now displays the real wxGTK splitter sash directly, styled with the exact neutral `RGB(240,240,240)` surface and a centered one-pixel dark/light etched rule.
- The obsolete visible child overlay is kept hidden on Linux, so it can no longer block native splitter hit-testing or drag feedback.
- A small Linux-only gutter is inserted after the fixed recovered Calculator surface, moving the History boundary to the right and preventing the final keypad column from crowding the separator.
- Linux frame-width calculations now include both that gutter and the native GTK sash extent, so the configured History content width is preserved.
- Windows keeps its existing recovered geometry, custom separator overlay, and splitter behavior unchanged.

## Windows embedded icon path — buildfix88

The platform refactor moved the Windows implementation from `src/platform.rs` to `src/platform/windows.rs`, but the compile-time `include_bytes!` path for `calc95.ico` retained its old relative depth. Windows compilation therefore searched for `src/calc95.ico` and failed before linking. The include now correctly resolves two levels upward to the project-root `calc95.ico`.

`tools/check_platform_separation.py` now resolves every literal Rust `include_bytes!` and `include_str!` path from the source file that contains it. This check scans all target-specific modules even when they are not compiled on the validation host, so a future Windows-only or Linux-only resource-path regression is detected statically.
