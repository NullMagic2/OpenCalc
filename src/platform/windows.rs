//! Windows-only Win32 integration.
...
// Common Controls v6 manifest -- that is exactly why ui/windows.rs has to set the
...
pub(super) fn viewer_missing_message() -> &'static str {
    "hlp-viewer.exe was not found. Place it beside the Calculator executable."
}
...
pub(super) fn append_system_help_candidates(
    ...
    if let Some(windir) = std::env::var_os("WINDIR") {
        let help = std::path::PathBuf::from(windir).join("HELP");
...
mod backend_tests {
    ...
    #[test]
    fn viewer_error_names_windows_companion() {
        assert!(viewer_missing_message().contains("hlp-viewer.exe"));
    }
}
