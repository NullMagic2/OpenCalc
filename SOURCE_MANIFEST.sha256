...
On Linux, `cargo build --release` produces `target/release/OpenCalc`. Place a native executable named exactly `hlp-viewer` beside that binary and keep `CALC_EN.HLP`/`CALC_EN.CNT`, `CALC_PT-BR.HLP`/`CALC_PT-BR.CNT`, and `CALC_ES.HLP`/`CALC_ES.CNT` in the adjacent `Help` directory.

## Source map

- `src/ui.rs` — original-layout wxDragon Calculator frame, native Calculator/History splitter, clickable calculation history, live localized menus, Undo/Redo integration, DPI-scaled controls, font aliasing, selector decorations, and preference commands.
...
- `src/locale.rs` — Windows NLS / Linux `LC_NUMERIC` decimal-symbol discovery and canonical/display conversion.
- `src/platform.rs` — clipboard, per-monitor DPI opt-in, Windows USER32 classic painters, Linux wxGTK/GTK3 classic-theme bridge, native What’s This? popup/tooltips, runtime Calculator icon application, Statistics activation guarding, and platform-native HLP Viewer launching.
